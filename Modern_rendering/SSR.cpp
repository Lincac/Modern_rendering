...
GLfloat SCR_WIDTH = 1024.0f;
GLfloat SCR_HEIGHT = 1024.0f;
...
int grass = 20;
vec3 lightpos = vec3(0.0f, 4.0f, -1.0f);
...
int main() {
	...
	Shader debugShader("shader/SSR/debug.vs", "shader/SSR/debug.fs");
	Shader SSRShader("shader/SSR/ssr.vs", "shader/SSR/ssr.fs");
	Shader gbufferShader("shader/SSR/gbuffer.vs", "shader/SSR/gbuffer.fs");
	Shader MipMapShader("shader/SSR/MipMap.vs", "shader/SSR/MipMap.fs");

	unsigned int sphere = loadTexture("resources/textures/awesomeface.png");
	unsigned int plane = loadTexture("resources/textures/wood.png");

	debugShader.use();
	debugShader.setInt("ssrcolor", 0);
	debugShader.setInt("Albedo", 1);

	gbufferShader.use();
	gbufferShader.setInt("Albedo", 0);

	MipMapShader.use();
	MipMapShader.setInt("u_depthBuffer", 0);
	...
	SSRShader.setInt("gParameter", 4);

	unsigned int SSRFBO;
	glGenFramebuffers(1, &SSRFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, SSRFBO);

	unsigned int SSRcolor;
	glGenTextures(1, &SSRcolor);
	glBindTexture(GL_TEXTURE_2D, SSRcolor);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, SCR_WIDTH, SCR_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, SSRcolor, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		mat4 projection = perspective(radians(camera.Zoom), SCR_WIDTH / SCR_HEIGHT, 0.1f, 1000.0f);
		...
		gbufferShader.setFloat("index", 0.0f);
		...
		gbufferShader.setMat4("view", view);
		gbufferShader.setMat4("projection", projection);
		gbufferShader.setFloat("index", 1.0f);
		glActiveTexture(GL_TEXTURE0);
		...
		int numLevels = 1 + (int)floorf(log2f(fmaxf(SCR_WIDTH, SCR_HEIGHT))); // 计算最大分辨率
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		SSRShader.use();
		SSRShader.setInt("maxLevel", numLevels - 1);
